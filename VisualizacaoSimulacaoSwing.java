...
public class VisualizacaoSimulacaoSwing extends JFrame {
    ...
    private JTextField txtNumeroCaminhoesPequenos; // Novo campo para número de caminhões pequenos
    ...
    private JButton btnCalcularMinimoCaminhoes;
    ...
    private void inicializarPosicoes() {
        ...
        if (simulador.getCaminhoesPequenos() != null) {
            for (int i = 0; i < simulador.getCaminhoesPequenos().tamanho(); i++) {
                ...
                if (caminhao != null && caminhao.getZonaAtual() != null) {
                    posicoesCaminhoesPequenos.put(caminhao.getId(), new Point(getPosicaoZona(caminhao.getZonaAtual())));
                }
            }
        }
    ...
    private void inicializarInterface() {
        SwingUtilities.invokeLater(() -> {
            ...
            btnCalcularMinimoCaminhoes = new JButton("Calcular Mínimo de Caminhões");
            btnCalcularMinimoCaminhoes.setFont(botaoFont);
            btnCalcularMinimoCaminhoes.setBackground(new Color(255, 165, 0));
            btnCalcularMinimoCaminhoes.setForeground(Color.WHITE);
            btnCalcularMinimoCaminhoes.setFocusPainted(false);
            btnCalcularMinimoCaminhoes.setBorder(BorderFactory.createEmptyBorder(8, 15, 8, 15));
            btnCalcularMinimoCaminhoes.addActionListener(e -> {
                new Thread(() -> {
                    int minimoCaminhoes = simulador.calcularMinimoCaminhoesGrandes(1440);
                    SwingUtilities.invokeLater(() -> {
                        JOptionPane.showMessageDialog(this, "Número mínimo de caminhões grandes necessários: " + minimoCaminhoes,
                                "Resultado", JOptionPane.INFORMATION_MESSAGE);
                        lblCaminhoesGrandesNecessarios.setText(String.valueOf(minimoCaminhoes));
                    });
                }).start();
            });
            ...
            painelControles.add(btnCalcularMinimoCaminhoes);
            ...
            painelMapa.setPreferredSize(new Dimension(900, 800)); // Aumentei a altura para permitir rolagem
            ...
            painelVisualizacaoCaminhoes = new JPanel() {
                ...
                protected void paintComponent(Graphics g) {
                    ...
                    int x = 50, y = 80, espacamentoHorizontal = 200, espacamentoVertical = 200; // Aumentei os espaçamentos
                    g2d.setFont(new Font("Segoe UI", Font.BOLD, 16));
                    ...
                    if (simulador != null && simulador.getCaminhoesPequenos() != null) {
                        synchronized (simulador.getCaminhoesPequenos()) {
                            for (int i = 0; i < simulador.getCaminhoesPequenos().tamanho(); i++) {
                                CaminhaoPequeno caminhao = simulador.getCaminhoesPequenos().obter(i);
                                if (caminhao == null) continue;
                                int coluna = i % 3; // Reduzi para 3 colunas para mais espaço
                                int linha = i / 3;
                    ...
                    y = y + (simulador.getCaminhoesPequenos().tamanho() / 3 + 1) * espacamentoVertical; // Ajuste dinâmico
                    g2d.setFont(new Font("Segoe UI", Font.BOLD, 16));
                    ...
                    // Ajuste dinâmico da altura do painel
            ...
            painelAbas.addTab("Mapa", scrollMapa); // Adicionada rolagem
            ...
            for (int i = 0; i < simulador.getCaminhoesPequenos().tamanho(); i++) {
                ...
                if (posAtual != null) {
                    if (caminhao.estaNaEstacaoTransferencia()) {
                        ...
                        Zona zona = caminhao.getZonaAtual();
                        if (zona != null) {
    ...
    private void atualizarInterface() {
            ...
            synchronized (simulador.getZonas()) {
                for (int i = 0; i < simulador.getZonas().tamanho(); i++) {
                    ...
                    if (zona != null && i < lblZonaInfo.length) {
                        lblZonaInfo[i].setText(String.format("Zona: %s | Geração: %.1f-%.1f t/dia | Acumulado: %.1f t",
                                zona.getNome(), ConfiguracaoSimulacao.GERACAO_LIXO_MIN[i], ConfiguracaoSimulacao.GERACAO_LIXO_MAX[i], zona.getLixoAcumulado()));
                    }
                }
            }
        }
        ...
        if (simulador.getCaminhoesPequenos() != null) {
            synchronized (simulador.getCaminhoesPequenos()) {
                for (int i = 0; i < 10; i++) { // Limite de 10 para evitar erros
    ...
    private void mostrarRelatorio() {
        ...
        painelRelatorio.setBorder(BorderFactory.createEmptyBorder(15, 15, 15, 15));
        painelRelatorio.setBackground(Color.WHITE);

        JTextArea textoResumo = new JTextArea();
        ...
        painelRelatorio.add(new JScrollPane(textoResumo), BorderLayout.NORTH);

        JTable tabelaEstatisticas = criarTabelaEstatisticas();
        painelRelatorio.add(new JScrollPane(tabelaEstatisticas), BorderLayout.CENTER);

        JPanel painelGraficos = new JPanel(new GridLayout(2, 1, 10, 10));
        painelGraficos.setBorder(BorderFactory.createTitledBorder("Gráficos (Placeholder)"));
        painelGraficos.setBackground(Color.WHITE);

        JLabel lblGrafico1 = new JLabel("Gráfico: Lixo Coletado ao Longo do Tempo (Implementar com JFreeChart)");
        lblGrafico1.setFont(new Font("Segoe UI", Font.PLAIN, 12));
        painelGraficos.add(lblGrafico1);
        ...
        painelRelatorio.add(painelGraficos, BorderLayout.SOUTH);

        dialog.add(painelRelatorio);
