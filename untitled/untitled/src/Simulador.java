...
public class Simulador {
    ...
    private double somaLixoParaAterro = 0.0;
    private int somaTempoEspera = 0;
    private int countTempoEspera = 0;
        ...
        while (tempoSimulacao < duracao) {
            ...
            synchronized (zonas) {
                for (int i = 0; i < zonas.tamanho(); i++) {
                    ...
                    if (zona != null) {
                        ...
                        if (tempoSimulacao % 60 == 0) {
                            System.out.println("[LOG] Zona " + zona.getNome() + " gerou " + String.format("%.4f", (lixoDepois - lixoAntes)) + " t de lixo. Total acumulado: " + String.format("%.4f", lixoDepois));
                        }
                    }
                }
            }
            ...
            synchronized (caminhoesPequenos) {
                for (int i = 0; i < caminhoesPequenos.tamanho(); i++) {
                    ...
                    if (caminhao != null && !caminhao.estaNaEstacaoTransferencia()) {
                        if (tempoSimulacao % 60 == 0) {
                            System.out.println("[LOG] Caminhão Pequeno " + caminhao.getId() + " está na zona " + caminhao.getZonaAtual().getNome() + ", carga: " + caminhao.getCargaAtual());
                        }
                        ...
                        if (caminhao.getCargaAtual() >= caminhao.getCapacidade() * 0.8) {
                            ...
                            if (estacao != null) {
                                ...
                                if (tempoSimulacao % 60 == 0) {
                                    System.out.println("[LOG] Caminhão Pequeno " + caminhao.getId() + " indo para Estação " + estacao.getId() + " (chegada em " + (tempoSimulacao + tempoViagem) + ")");
                                }
                                estacao.adicionarCaminhaoPequeno(caminhao, tempoSimulacao + tempoViagem);
                                caminhao.setNaEstacaoTransferencia(true);
                            }
                        }
                    } else if (caminhao != null && caminhao.estaNaEstacaoTransferencia()) {
                        if (tempoSimulacao % 60 == 0) {
                            System.out.println("[LOG] Caminhão Pequeno " + caminhao.getId() + " está na estação aguardando processamento.");
                        }
                    }
                }
            }
            ...
            synchronized (estacoesTransferencia) {
                for (int i = 0; i < estacoesTransferencia.tamanho(); i++) {
                    ...
                    if (estacao != null) {
                        if (tempoSimulacao % 60 == 0) {
                            System.out.println("[LOG] Processando Estação " + estacao.getId() + ", Fila: " + estacao.getFilaCaminhoesPequenos().tamanho());
                        }
            ...
            // Após processar as estações, registre o lixo coletado para o gráfico
            if (visualizacaoIniciada) {
                VisualizacaoSimulacaoSwing.informarLixoColetado(somaLixoParaAterro);
            }
        }
    ...
    public double getTempoMedioEspera() {
        if (countTempoEspera == 0) return 0.0;
        return (double) somaTempoEspera / countTempoEspera;
    }

    public double getLixoTotalColetado() {
        return somaLixoParaAterro;
    }
