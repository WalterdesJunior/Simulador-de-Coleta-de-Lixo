...
public class EstacaoTransferencia {
    ...
    public boolean processar(int tempoAtual, ListaEncadeada<Double> lixoParaAterro) {
        ...
        if (!filaCaminhoesPequenos.estaVazia() && caminhaoGrandeAtual != null) {
            ...
            if (tempoEspera >= 0) {
                System.out.println("[LOG] Caminhão Pequeno " + caminhao.getId() + " será processado na Estação " + id + ". Tempo de espera: " + tempoEspera);
                ...
                System.out.println("[LOG] Caminhão Pequeno " + caminhao.getId() + " descarregou " + lixoDescarregado + " t e retorna para zona " + caminhao.getZonaFixa().getNome());
                ...
                // Em vez de sortear uma nova zona, retorna para a zona fixa
                caminhao.setZonaAtual(caminhao.getZonaFixa());
